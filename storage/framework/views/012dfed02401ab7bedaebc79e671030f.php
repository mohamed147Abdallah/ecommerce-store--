    ...
    <div class="flex flex-col md:flex-row min-h-screen">
        ...
        <nav class="fixed bottom-0 md:top-0 md:left-0 w-full md:w-24 md:h-screen h-16 glass-strong z-50 flex md:flex-col justify-between items-center py-0 md:py-8 px-6 md:px-0 md:border-r md:border-t-0 border-t border-white/5">
            ...
            <div class="hidden md:flex flex-col gap-6 items-center">
                 <a href="<?php echo e(route('cart.index')); ?>" class="relative group cursor-pointer text-white">
            ...
            <div class="flex-1 max-w-[1600px] mx-auto w-full px-6 md:px-16 py-12 md:py-24">
                ...
                <div class="flex items-end justify-between border-b border-white/10 pb-8 mb-12 animate-entry delay-100">
                    ...
                    <div class="hidden md:block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">
                        Total Items: <span id="header-count" class="text-white"><?php echo e(count(session('cart', []))); ?></span>
                    </div>
                </div>

                <!-- Empty State (Hidden by default if items exist) -->
                <div id="empty-cart-message" class="<?php echo e(session('cart') && count(session('cart')) > 0 ? 'hidden' : 'flex'); ?> flex-col items-center justify-center py-32 border border-dashed border-white/10 bg-[#0a0a0a] rounded-3xl animate-entry delay-200">
                    ...
                    <a href="<?php echo e(route('products.index')); ?>" class="bg-white text-black px-10 py-4 text-xs font-black uppercase tracking-widest hover:bg-gray-200 rounded-full transition-all duration-300">
                        Start Shopping
                    </a>
                </div>
                    ...
                    <div id="cart-content" class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
                        ...
                        <div id="cart-items-container" class="lg:col-span-8 space-y-6 animate-entry delay-200">
                            <?php $__currentLoopData = session('cart'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $id => $details): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div id="cart-row-<?php echo e($id); ?>" class="group relative flex flex-col sm:flex-row bg-[#0a0a0a] border border-white/10 rounded-3xl overflow-hidden hover:border-white/30 transition-all duration-300" data-price="<?php echo e($details['price']); ?>">
                                    
                                    <!-- Image -->
                                    <div class="w-full sm:w-48 h-48 sm:h-auto relative overflow-hidden">
                                        <a href="<?php echo e(route('products.show', $id)); ?>" class="block w-full h-full">
                                            <img src="<?php echo e($details['image'] ? asset('storage/'.$details['image']) : 'https://via.placeholder.com/150/171717/525252'); ?>" 
                                    ...
                                    <div class="flex-1 p-6 sm:p-8 flex flex-col justify-between">
                                        <div class="flex justify-between items-start">
                                            <div>
                                                <h3 class="text-2xl font-black text-white uppercase tracking-tighter mb-1">
                                                    <a href="<?php echo e(route('products.show', $id)); ?>" class="hover:text-gray-300 transition-colors"><?php echo e($details['name']); ?></a>
                                                </h3>
                                               
                                            </div>
                                            
                                            <!-- Remove Button (AJAX) -->
                                            <form action="<?php echo e(route('cart.remove', $id)); ?>" method="POST" class="ajax-cart-form" data-type="remove" data-id="<?php echo e($id); ?>">
                                        ...
                                        <div class="flex items-end justify-between mt-6">
                                            ...
                                            <div class="flex items-center gap-1 bg-white/5 rounded-full p-1 border border-white/10">
                                                <form action="<?php echo e(route('cart.decrease', $id)); ?>" method="POST" class="ajax-cart-form" data-type="decrease" data-id="<?php echo e($id); ?>">
                                                ...
                                                <span id="qty-<?php echo e($id); ?>" class="w-8 text-center font-bold text-white text-xs"><?php echo e($details['quantity']); ?></span>
                                                
                                                <form action="<?php echo e(route('cart.add', $id)); ?>" method="POST" class="ajax-cart-form" data-type="increase" data-id="<?php echo e($id); ?>">
                                                    <?php echo csrf_field(); ?>
                                            ...
                                            <div class="text-right">
                                                <div id="subtotal-<?php echo e($id); ?>" class="text-2xl font-bold text-white tracking-tight">$<?php echo e(number_format($details['price'] * $details['quantity'], 2)); ?></div>
                                                <div class="text-[10px] text-gray-500 uppercase tracking-widest">$<?php echo e($details['price']); ?> each</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="lg:col-span-4 animate-entry delay-300">
                            <div class="bg-[#0a0a0a] border border-white/10 rounded-3xl p-8 sticky top-6 shadow-2xl">
                                <h3 class="text-sm font-black text-white uppercase tracking-widest mb-8 border-b border-white/10 pb-4 flex justify-between items-center">
                                    Summary <span id="summary-count" class="text-gray-600 text-[10px]"><?php echo e(count(session('cart', []))); ?> ITEMS</span>
                                </h3>
                                
                                <div class="space-y-4 mb-8">
                                    <div class="flex justify-between text-xs font-bold uppercase tracking-wide">
                                        <span class="text-gray-500">Subtotal</span>
                                        <span id="summary-subtotal" class="text-white">$<?php echo e(number_format($total ?? 0, 2)); ?></span>
                                    </div>
                                    <div class="flex justify-between text-xs font-bold uppercase tracking-wide">
                                ...
                                <div class="flex justify-between items-end mb-8 pt-6 border-t border-white/10">
                                    ...
                                    <span id="summary-total" class="text-4xl font-black text-white tracking-tighter">$<?php echo e(number_format($total ?? 0, 2)); ?></span>
                                </div>
...
</html><?php /**PATH C:\Users\ASUS\OneDrive\Desktop\laravel\ecommerce-store\resources\views/cart/index.blade.php ENDPATH**/ ?>