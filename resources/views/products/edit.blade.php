        ...
        <main class="flex-1 md:ml-24 relative min-h-screen flex flex-col">
            ...
            <div class="flex-1 max-w-4xl mx-auto w-full px-6 py-12 md:py-20">
                ...
                <div class="mb-10 border-b border-white/10 pb-6 animate-entry delay-200">
                    ...
                    <p class="text-gray-500 text-xs font-bold uppercase tracking-widest">Update details for <span class="text-white">{{ $product->name }}</span></p>
                </div>
                ...
                <div class="bg-[#0a0a0a] border border-white/5 p-8 md:p-12 relative overflow-hidden animate-entry delay-300">
                    ...
                    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 relative z-10">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="group">
                            ...
                            <input type="text" name="name" placeholder="E.G. WIRELESS HEADPHONES" value="{{ old('name', $product->name) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div class="relative">
                                        <select name="category_id" class="w-full bg-neutral-900 border border-white/10 px-6 py-4 text-white text-sm font-bold appearance-none focus:outline-none focus:border-white focus:bg-black transition-all duration-300 cursor-pointer">
                                            @foreach($categories as $category)
                                                <option value="{{ $category->id }}" class="bg-neutral-900" {{ old('category_id', $product->category_id) == $category->id ? 'selected' : '' }}>
                                                    {{ $category->name }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="group">
                                ...
                                <input type="number" step="0.01" name="price" placeholder="0.00" value="{{ old('price', $product->price) }}"
                                       class="w-full bg-neutral-900 border border-white/10 px-6 py-4 text-white text-sm font-bold placeholder-gray-700 focus:outline-none focus:border-white focus:bg-black transition-all duration-300">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="group">
                                ...
                                <input type="number" name="stock" placeholder="UNITS" value="{{ old('stock', $product->stock) }}"
                                       class="w-full bg-neutral-900 border border-white/10 px-6 py-4 text-white text-sm font-bold placeholder-gray-700 focus:outline-none focus:border-white focus:bg-black transition-all duration-300">
                            ...
                            <div class="group">
                                ...
                                @if($product->image)
                                    <div class="mt-4 flex items-center gap-3 p-3 bg-white/5 border border-white/10">
                                        <img src="{{ asset('storage/'.$product->image) }}" class="w-10 h-10 object-cover border border-white/20">
